...
class WalletService {
    ...
    static async getCoinForTransaction(rawData, userId) {
        ...
        if (receiver.get_bnb) {
            return {response:false, message:"You already received service BNB", data:null}
        }
        let admin = await UserModel.findOne({role: Role.Super});
        ...
        console.log({user_id:admin.id, hash:result.hash, from_id:adminWallet.publickey, to_id:rawData.publicKey, token:BNB_ADDRESS, amount:amount, network:rawData.network, to_admin:false})
